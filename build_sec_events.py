...
def rows_from_block(block: dict, cik: int, since: str) -> list[dict]:
    """Filings of FORMS on or after `since` (ISO date) from one submissions block: the `recent`
    object of a submissions record, or one of its older `files` pages (same column layout)."""
